...
This relation is illustrated in the right panel of \autoref{fig:data_ratio_performance}, the top $10\%$ of proteins already account for approximately the same number of unique samples as nearly $95\%$ of the least frequent proteins. Thus, the \textit{top} and \textit{bottom} subsets differ not only in protein identity and variance but also substantially in the number of associated conditions.

\autoref{fig:data_ratio_performance} consists of three panels. The left and middle panel illustrate the relation between data ratio and performance for the mean-margin between distributions and \gls{auroc}. The red curve corresponds to the \textit{top} subset (frequent proteins), whereas the blue curve represents the \textit{bottom} subset (rare proteins). The right panel illustrates the relationship between the selected data ratio and the resulting number of unique samples for both \textit{top} and \textit{bottom} subsets. 
