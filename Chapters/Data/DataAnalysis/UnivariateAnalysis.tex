...
\paragraph{Protein features}\label{sec:data_analysis/univariate_analysis/protein}
...
First, all polymer entities in the corresponding \gls{pdb} entry were filtered to retain only protein chains, i.e. entities annotated as polypeptide(L) or polypeptide(D), which denote polypeptide chains composed of L- or D-amino acids, respectively (mirror-image stereoisomers).
Nucleotide and ribonucleotide chains were discarded. Second, for each remaining entity, the amino-acid sequence was repeated according to its stoichiometric multiplicity, inferred from the number of strand identifiers associated with that entity. Finally, all repeated chain sequences were concatenated to form the complete protein sequence used as model input.
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.32\linewidth}
		...
		\caption[Atom count Distribution]{ }
	...
	\begin{subfigure}[t]{0.32\linewidth}
		...
		\caption[Atom count vs. Sequence Length]{ }
		\label{fig:atomCountsVsSeqLength}
	\end{subfigure}
	\caption[Sequence Length and Atom count Distribution]{Visualization of the distribution of sequence lengths (\autoref{fig:seqLength}) and deposited atom counts (\autoref{fig:atomCounts}). Most proteins have sequences between 50 and 400 residues, with models containing roughly 1,000–8,000 atoms. The relationship is largely linear as seen in \autoref{fig:atomCountsVsSeqLength}, though errors and different asymmetric cell parameters cause deviations in slope.}
	\label{fig:sequenceAndAtomCount}
\end{figure}

The distribution of protein sequence lengths is shown in \autoref{fig:seqLength}. Most proteins in the dataset fall within a biologically plausible range of approximately 50 to 700 amino-acid residues. Although the mean sequence length is 784 residues, this value is heavily skewed by few extreme outliers: the maximum observed sequence length is 87,120 residues, and the standard deviation is correspondingly large at 1,198 residues. More robust statistics provide a clearer picture of the dataset: the median sequence length is 474 residues and the mode is 306 residues, which more accurately reflect the typical protein size encountered in structural biology.
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.495\linewidth}
		...
		\caption[Apolar and Polar Surface Atoms]{ }
	...
	\begin{subfigure}[t]{0.495\linewidth}
		...
		\caption[Surface feature comparison]{ }
		\label{fig:surfaceDistribution}
	\end{subfigure}
	\caption[Derived surface feature distributions]{\autoref{fig:apolarVsPolar} shows the distributions of derived surface features. 
		Apolar and polar surface atoms show approximately normal distributions with comparable overall exposure. 
		Surface roughness, sidechain fraction, and SASA skewness are centered around~1 as seen in \autoref{fig:surfaceDistribution}, while the remaining surface descriptors cluster near zero or show slight positive shifts.}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[b]{0.42\linewidth}
		...
		\caption[Number of compounds in a cocktail]{ }
	...
	\begin{subfigure}[b]{0.55\linewidth}
		...
		\caption[Most common compounds with concentration]{ }
		\label{fig:mostCommonChemicals}
	\end{subfigure}
	\caption[General Cocktail composition]{\autoref{fig:nCompoundsDistribution} shows the number of compounds in a cocktail which on average is 3. Some conditions specify as much as 10 chemicals. \autoref{fig:mostCommonChemicals} depicts the most common compounds across all cocktails feature multiple \glspl{peg}, buffers and salts. For the majority of the compounds a concentration value is given.}
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.48\linewidth}
		...
		\caption[Most common chemicals]{ }
		\label{fig:most_common_chem_with_concentration_point}
	\end{subfigure}
	\caption[Most common compounds with concentrations]{The relative concentration ($w/v$ and $v/v$) at which different \glspl{peg} occur differs as seen in \autoref{fig:most_common_pegs_with_concentration_point}. The spread ($\pm$ 1 SD)differs as well with some \glspl{peg} having a higher spread of values (e.g. \gls{peg} 400) than others (e.g. \gls{peg} 20 000). The same holds for other compounds as depicted in \autoref{fig:most_common_chem_with_concentration_point}. In comparison, buffers such as tris, HEPES, MES have both a lower average concentration and a much smaller spread than salts such as ammonium sulfate or sodium chloride/acetate/citrate.}
	\label{fig:mostCommonCompoundsAndConcentration}
\end{figure}

A more fine-grained view of compound usage is provided in \autoref{fig:mostCommonCompoundsAndConcentration}. The distribution of molecular weights for \glspl{peg}, shown in \autoref{fig:most_common_pegs_with_concentration_point}, exhibits a clear bias: the five most frequently used \gls{peg} variants account for approximately 60\% of all \gls{peg} occurrences in the dataset. Despite this strong preference for specific molecular weights, the concentrations at which these \glspl{peg} are used vary substantially. Lighter \glspl{peg} such as \gls{peg}~400, 550, 1000, 1500, and 2000 are typically present at higher concentrations (above 21\%), whereas heavier \glspl{peg}—including \gls{peg}~6000, 8000, 10 000, and 20 000—tend to be used at noticeably lower concentrations (below 17\%). The model must learn to reproduce not only which \gls{peg} molecular weight is appropriate, but also the concentration at which it should be applied. The spread of the concentration at which certain \glspl{peg} appear differs. Some standard deviations cover much larger ranges than others. This can be seen when comparing \gls{peg} 400 and \gls{peg} 20 000. 
...
Second, the concentration ranges for these non-\gls{peg} compounds are considerably broader. It is noticeable that the spread of buffers is much smaller than for salts. One standard deviation of HEPES, tris, MES, tris-hcl and bis tris typically fall in range of 100 to 300. The spread for salts is much larger with a one standard deviation covering a range up to 1.5 moles/liter.
...
\begin{figure}[h!]
	...
	\begin{subfigure}[t]{0.48\linewidth}
		...
		\caption[Cocktail classes]{ }
		\label{fig:commonChemClasses}
	\end{subfigure}
	\label{fig:cocktailAnalysis}
	\caption[Cocktail Analysis]{\autoref{fig:mostcommoncocktails} depicts the most frequent specific chemical combinations found in the crystallization conditions, showing the dominant pairs and triplets of reagents across the dataset. Whereas \autoref{fig:commonChemClasses} shows the most frequent combinations of chemical classes occurring together in crystallization setups, highlighting the typical pairing patterns between polymers, buffers, and salts.}
\end{figure}
