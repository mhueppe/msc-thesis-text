...
The following chapter presents an introduction to the \gls{pdb} and how the data was \hyperref[sec:protein_database/acquisition]{acquired}, \hyperref[sec:protein_database/format]{is formatted}, and \hyperref[sec:protein_database/structure]{structured} to enable data preprocessing and analysis. Given the format of the data extensive parsing is applied as detailed in \autoref{sec:data/data_parsing}. \autoref{sec:data_analysis} presents the data analysis that was done after applying parsing and normalization. 
\autoref{sec:crims} presents an additional dataset, which was acquired, cleaned and analyzed to further validate model performance. 

\paragraph{Acquisition}\label{sec:protein_database/acquisition}
...
The data items are in the format of '\_' + category name + '.' + attribute name. Data categories can be either saved in either key-value or in tabular format. These can be easily parsed into a column vector as is in CSV files. For example the crystallization conditions are saved in key-value format an example is depicted in \autoref{lst:example_file} in \autoref{app:data}.



A category is stored in tabular format when a token defines multiple values. In this case, loop\_ is followed by rows of data-item names, with data values separated by whitespace. Notably, the protein’s structural information i.e., the atom\_sites category is represented in this format, as shown in \autoref{lst:feature_example} in \autoref{app:data}.
...
\begin{figure}
	...
	\caption[Restructured protein database schema]{
		Visualization of the restructured database format derived from the \texttt{protein.cif} file. 
		The \texttt{protein.cif} serves as the point of origin and contains all available information for a given protein. 
		
		In the restructured schema, each CIF category is stored as a separate table whose columns correspond to the category attributes, while each row represents a protein entry. To enable relational operations, all category tables share a common \texttt{entry\_id} column.
	}
