\section{Extreme Gradient Boosting}\label{sec:xgBoost}
...
\subsection{Regularized Objective Function}\label{sec:xgBoost/regularization}
...
The parameter $\gamma$ penalizes the number of leaves, while $\lambda$ controls the magnitude of the leaf weights. This regularization encourages simpler tree structures and improves generalization performance. Both $\gamma$ and $\lambda$ are hyparameters that can be tuned for during training. The standard default for $\gamma$ is 0 while for $\lambda$ it is 1. While this means that tree complexity is per default not explicitly penalized through this regularization term trees do still do not grow arbitrarily large due to other regularization techniques. Specifally, \gls{xgBoost} incorporates the following methods to avoid overfitting:
...
\subsection{Second-Order Gradient Optimization}
...
\subsection{Tree Structure Optimization}
