\subsection{Surface Embedding}\label{sec:methods/feature_engineering/surface_embedding}
...
\gls{masif} demonstrated that such surface descriptors are powerful for predicting protein-protein interaction sites, ligand binding regions, and molecular complementarity. However, mesh-based representations come with drawbacks. They require expensive pre-computation of meshes and vertex-level features, geometric convolutions on meshes are computationally heavy, and the pipeline is sensitive to mesh quality.

To address these limitations \textcite{Sverrisson2020} proposed \gls{dmasif} as a differential, point-based alternative. Instead of precomputing meshes, \gls{dmasif} generates point clouds directly from atomic coordinates using differentiable surface estimation techniques. This eliminates the need for mesh extraction and feature preprocessing.
...
\autoref{fig:bindingSitePrediction} shows downstream binding site predictions made from the ProteinMAE embedding. As seen the binding sites are very clear distinguished from the rest of the protein. 
