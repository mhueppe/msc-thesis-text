\subsection{Training Procedure}\label{sec:methods/training/pipeline}

The model is trained using a contrastive ranking objective within a two-module architecture. The goal is to learn a shared embedding space in which compatible protein--condition pairs have high similarity, while incompatible pairs are separated.
...
As described in \autoref{sec:methods/model_architecture} for each batch, protein and condition inputs are encoded independently using the two modules:
\begin{equation}
